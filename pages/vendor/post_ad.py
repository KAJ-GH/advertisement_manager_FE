...
def post_ad_page():
    ...
    with ui.row().classes("w-full h-screen flex flex-row justify-between items-center"):
        with ui.column().classes("w-[20%] h-full"):
            ...
            #with ui.element("div").classes("w-full h-screen relative").style(
               # "background-image: url(/assets/laptop-1478822_640.jpg); background-size: cover; background-position: center; background-repeat: no-repeat;"
            #):
    ...
    with ui.card().classes(
        ...
        with ui.row().classes("w-full gap-3 mt-4"):
            ...
            ui.button(
                "Cancel", on_click=lambda: ui.navigate.to("/vendor/vendor_ads")
            ).classes( 
